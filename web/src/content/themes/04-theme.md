...
blocks:
  - type: BlockStats
    subtitle: "01"
    title: "期待できる導入効果"
    desc: "「つなげモン ハブ」によるデータの一元化は、日々の業務を確実に効率化します。"
    items:
      - number: "約50%"
        label: "転記作業の削減"
        desc: "各システム間での手打ち作業やコピペを自動化し、本来の業務に集中できる時間を増やします。"
      - number: "防止"
        label: "データの正確性"
        desc: "システム間の直接データ連携により、ヒューマンエラーによる転記ミスや入力漏れを防ぎます。"
      - number: "最適化"
        label: "システム導入費"
        desc: "巨大で高額なERPに総入れ替えするのではなく、既存システムを活かしつつハブを作ることでコストを抑えます。"
  ...
  - type: BlockFAQ
    ...
    items:
      ...
      - q: "データのバックアップ体制はどうなっていますか？"
        a: "運用するサーバー（VPS）側で定期的な自動バックアップを行っております。万が一のシステムトラブル時にも、バックアップデータから復旧可能な体制を整えています。"
