...
blocks:
  - type: BlockText
    ...
    paragraphs:
      ...
      - "つなげモン シミュレータは、社内のあらゆるデータを結集し、あなたの会社専用のデジタルツイン（仮想モデル）を構築。何千通りものシナリオをシミュレータが瞬時にテストし、最も安全で利益の出るルートを提示します。"
  ...
  - type: BlockReasoningGrid
    ...
    items:
      - num: "POINT 01"
        title: "自社データに最適化された機械学習モデル"
        desc: "一般的な予測ツールとは異なり、貴社独自の生産能力、リードタイム、原価構造を完全に学習した専用モデルを構築します。"
        image: "theme06_feat1.webp"
      - num: "POINT 02"
        title: "ボトルネックの自動発見機能"
        desc: "目標売上を入力すると、「3ヶ月後に第2倉庫の保管スペースが足りなくなる」といった未来のボトルネックをシミュレータが先に指摘してくれます。"
        image: "theme06_feat2.webp"
      - num: "POINT 03"
        title: "経営陣が使いやすいダッシュボード"
        desc: "複雑な計算結果を、利益率や稼働率といった分かりやすいグラフで表示。「A案」と「B案」の比較もワンクリックで行えます。"
        image: "theme06_feat3.webp"

  - type: BlockSteps
    subtitle: "03"
    title: "シミュレーション環境構築のステップ"
    items:
      - title: "【Step1】お試しレポート（スポット分析）"
        desc: "いきなりシステムを導入するのではなく、まずは貴社の過去データをセキュアな環境でお預かりし、機械学習を用いた単発の「分析レポート」を作成してご提示します。"
      - title: "【Step2】シミュレータ環境の構築"
        desc: "レポート結果にご納得いただけた場合、貴社専用のシミュレータ環境を構築し、社内で自由にシナリオテストができるシステムを導入します。"
      - title: "【Step3】自社運用と伴走支援"
        desc: "導入後も私たちが伴走し、シミュレータを活用した経営判断や、変数のチューニングなどを継続的にサポートいたします。"
