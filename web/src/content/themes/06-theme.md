...
pricing:
  free_trial: "過去3年分のデータを入力し、実際の直近の売上をどれだけ正確に予測できたかを確認する精度検証テストを無償で提供します。"
  initial: "300"
  monthly: "80"
button:
  type: "link"
...
blocks:
  ...
  - type: BlockStats
    ...
    items:
      - number: "92%"
        label: "需要予測の精度"
        desc: "過去の販売データと外部要因（天候やトレンド）を掛け合わせ、高精度な予測を実現。"
      - number: "10,000+"
        label: "同時シナリオテスト"
        desc: "価格変動、人員欠如、物流ストップなど、ありとあらゆるリスクパターンを瞬時に計算。"
      - number: "0円"
        label: "失敗時のサンクコスト"
        desc: "現実世界で失敗する前にシミュレータ内で失敗させることで、致命的な損失を防ぎます。"
  ...
  - type: BlockCTA
    ...
    desc: "過去のデータを用いた無償の精度検証テストで、AIの予測力をお確かめください。"
    button:
      text: "シミュレータについて相談する"
